...
# Load secrets (TS_AUTHKEY_*, etc.) from the gitignored .env if present
if [[ -f .env ]]; then
    set -a
    source ./.env
    set +a
...
hostnamectl set-hostname vps-relay
...
# hello-world can't abort the whole bootstrap before the relay stack + sshd purge.
if [[ -t 1 ]]; then
...
# own sshd, so Tailscale lives at the host level. Tagged tag:vps-admin, reachable
# only via Tailscale SSH (--ssh). LISH is break-glass.
...
### Relay data-plane stack (Tailscale container + Traefik) ###
# Idempotent: re-running converges the stack. Reads TS_AUTHKEY_RELAY from .env
# (docker compose auto-loads .env in this directory for interpolation).
docker compose up -d
